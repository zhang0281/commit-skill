...
def command_plan(args: argparse.Namespace) -> int:
    repo = repo_root(args.repo)
    payload = ok_payload(**build_plan(repo, args.include, args.exclude, args.split_mode, args.sign_mode))
    maybe_write_output(payload, args.out)
    return 0
...
def command_apply_plan(args: argparse.Namespace) -> int:
    ...
    if repo != plan["repo"]:
        raise SkillError(ErrorCode.PLAN_FILE_INVALID, "--repo 与计划 JSON 中的 repo 不一致", {"repo": repo, "plan_repo": plan["repo"]})
...
def add_commit_parser(sub: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    parser.add_argument("--type", required=True, choices=["feat", "fix", "docs", "refactor", "test", "chore", "style", "perf"])
